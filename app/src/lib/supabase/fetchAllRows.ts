 ...
 * project's `db-max-rows` limit (1000 by default) — an unbounded `.select()`
 * silently truncates past that limit instead of erroring, so any checklist
 * with more species than the limit would have some queries see the full
 * list (whichever ones page) and others see an arbitrary truncated slice
