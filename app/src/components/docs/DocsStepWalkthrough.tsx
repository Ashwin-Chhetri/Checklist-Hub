...
interface WalkthroughNode {
  title: string;
  body?: ReactNode;
  /** Swaps to this screen when clicked. Omit to point at whatever screen is already showing. */
  screen?: WalkthroughScreen;
  /** Where the arrow points, as a percentage (0-100) of the current screenshot. Omit for an inert entry (no click, no arrow) — e.g. a plain sub-bullet with nothing to swap to or point at. */
  target?: Point;
}

interface WalkthroughStep extends WalkthroughNode {
  body: ReactNode;
  subSteps?: WalkthroughNode[];
}

/** Same "i" / "i-j" convention DocsStepExplorer uses for its own activeKey. */
function stepKey(i: number, j?: number) {
  return j === undefined ? `${i}` : `${i}-${j}`;
}

/** Resolves an activeKey back to its node (step or sub-step). */
function resolveNode(steps: WalkthroughStep[], key: string): WalkthroughNode | undefined {
  const [i, j] = key.split("-").map(Number);
  const step = steps[i];
  if (!step) return undefined;
  return j === undefined ? step : step.subSteps?.[j];
}

/** A step/sub-step is only interactive if clicking it actually changes something. */
function isInteractive(node: WalkthroughNode) {
  return Boolean(node.screen || node.target);
}

// A distinct, stable numeric seed per key, for picking the arrow's template shape.
...
export default function DocsStepWalkthrough({
  defaultScreen,
  steps,
  variant = "dialog",
  hint,
}: {
  defaultScreen: WalkthroughScreen;
  ...
  const [currentScreen, setCurrentScreen] = useState<WalkthroughScreen>(steps[0]?.screen ?? defaultScreen);
  ...
  // Tracks which key the on-screen arrow currently belongs to, so a resize
  // (same node, arrow just needs to glide to a new position) can be told
  // apart from an actual step/sub-step change (old arrow should fade out).
  ...
  const handleSelect = (key: string, node: WalkthroughNode) => {
    setActiveKey(key);
    const nextScreen = node.screen ?? defaultScreen;
  ...
  useLayoutEffect(() => {
    ...
    const recompute = () => {
      ...
      frame = requestAnimationFrame(() => {
        const activeNode = resolveNode(steps, activeKey);
        const target = activeNode?.target;
        if (!target) return;

        const root = rootRef.current;
        const itemEl = itemRefs.current.get(activeKey);
        ...
        // The screen's own `variant` wins; otherwise it falls back to the
        // explorer's page-level default — same fallback FramedScreenshot
        // itself uses to pick which `object-position` actually renders.
        const contentRect = resolveContentRect(contentBox, currentScreen.aspect, currentScreen.variant ?? variant);
        ...
        const start: Point = isDesktop
          ? // Screen sits left of the steps: originate from the item's own
          ...
          : // Stacked: screen sits above the steps, so originate from the
            // item's top border, the side facing the screen.
            { x: itemRect.left + itemRect.width / 2 - rootRect.left, y: itemRect.top - rootRect.top };

        const built = buildArrow(start, end, seedFromKey(activeKey));
        const isNewStep = lastKeyRef.current !== activeKey;
        lastKeyRef.current = activeKey;

        setArrows((prev) => {
          if (!isNewStep) {
            // Same node, just a re-layout (resize/breakpoint change) — jump
            // the live arrow straight to its new geometry, no replay.
            return prev.map((a) => (a.exiting ? a : { ...a, ...built }));
          }
          // Node changed: ease the old arrow out, ease the new one in.
          prev.forEach((a) => {
  ...
  }, [activeKey, imageReady, isDesktop, currentScreen.aspect, steps]);
    ...
    >
      <div className="w-full max-w-[95%] mx-auto lg:mx-0">
        <FramedScreenshot
          ...
          variant={currentScreen.variant ?? variant}
        ...
        {steps.map((step, i) => {
          ...
          return (
            <li
              ...
              onClick={interactive ? () => handleSelect(key, step) : undefined}
                ...
                {step.subSteps && (
                  <ol className="mt-4 space-y-2">
                    {step.subSteps.map((sub, j) => {
                      const subKey = stepKey(i, j);
                      const isSubActive = activeKey === subKey;
                      const subInteractive = isInteractive(sub);
                      return (
                        <li
                          key={sub.title}
                          ref={(el) => {
                            if (el) itemRefs.current.set(subKey, el);
                            else itemRefs.current.delete(subKey);
                          }}
                          onClick={
                            subInteractive
                              ? (e) => {
                                  e.stopPropagation();
                                  handleSelect(subKey, sub);
                                }
                              : undefined
                          }
                          className={`rounded-sm border px-2 py-1.5 transition-colors ${
                            subInteractive ? "cursor-pointer" : ""
                          } ${
                            isSubActive
                              ? "bg-surface-container-low border-primary"
                              : subInteractive
                        ...
                        >
                          <span className="flex items-baseline gap-2">
                            <span
                              className={`font-code-md text-[12px] font-bold shrink-0 transition-colors ${
                                isSubActive || !subInteractive ? "text-primary" : "text-secondary/50"
                              }`}
                            >
                              {i + 1}.{j + 1}
                            </span>
                          ...
                          {sub.body && (
                            <p className="font-body-sm text-[13px] text-secondary mt-1 ml-[26px]">{sub.body}</p>
                          )}
                        </li>
                      );
                    })}
