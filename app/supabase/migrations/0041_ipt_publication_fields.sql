-- Publish module, IPT handoff step 2: two more fields recording what
-- happened once a checklist's DwC-A package was actually published through
-- IPT (gbif_doi/gbif_publication_year/gbif_citation already exist from
-- 0035) — the GBIF Registry dataset UUID, and when the IPT publish step was
-- recorded in ChecklistHub. Both are write-after-the-fact, same as the 0035
-- fields: ChecklistHub never mints or looks these up itself.

alter table checklist_metadata
  add column gbif_dataset_uuid text,
  add column ipt_published_at timestamptz;
