-- Adds region_pin to checklists. The wizard's RegionInput step1 component
-- already collects a pin code in its UI but it was never persisted anywhere
-- (CreateChecklistInput/Checklist had no column for it) — additive only.

alter table checklists add column region_pin text;

-- Update create_checklist_with_species to also store region_pin (same
-- function as 0011, with region_pin added to the checklists insert).
